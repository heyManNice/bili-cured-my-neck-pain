...
### 缩略图导航
- 160×90 缩略图显示当前视频画面，带方向字母和底部控制栏的蓝色多边形对应播放器真实可见范围
- 旋转后自动换算视频坐标，**拖拽视口框**或点击缩略图即可自由平移画面
- 平移不限制画面边界，可随时通过“重置位置”回到中心
...
| 源文件 | 行数 | 说明 |
|--------|------|------|
| `src/scripts/ctrl-rotate.ts` | - | 核心控制器 |
| `src/scripts/video-geometry.ts` | - | 旋转、缩放与导航坐标计算 |
